...
func TestSurvivalAvoidsTournamentCorridor(t *testing.T) {
	...
	for _, position := range saved.Cases {
		...
		routePolicy := policy
		routePolicy.EscapeDepth = 24
		if got := selectMove(position.Request, routePolicy); got != "left" && got != "down" {
			t.Fatalf("body-route policy chose %s into the closing corridor; want left or down", got)
		}
